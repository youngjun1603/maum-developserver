...
>
> 전 서비스 백로그의 **금지** 항목에도 *"CTS 개발(명시적 재개 시에만)"* 이 올라 있다.
>
> **루트 `CLAUDE.md` "남은 작업 (백로그)" 절 원문**(코드에서 복원 · 근거: 루트 `CLAUDE.md` L77~L82) — 레포에 남은 백로그는 이것이 전부이며, 이 워커에 직접 걸리는 항목은 **2개**다:
>
> > 전 서비스 남은 작업은 메모리 **`project_maum_backlog`** 한 곳에 모아 둔다. 새 작업 지시가 오면 여기부터 확인할 것.
> > - **바로 가능**: 폐기된 상담사 승인 레거시 코드 제거 / **주간 리포트 메일 실수신 검증(사용자 동의 후)** ← **이 워커**
> > - **데이터 보고 판단**(2026-08-09경): 마음게임 콘텐츠 확장 — 어드민 🔁 루프 탭에서 검사↔게임 루프가 도는지 확인 후
> > - **선행조건 대기**: 앱화·통합해석 상품화·연동형 통합결제 → 모두 **토스 실결제 반영 후**
> > - **금지**: 커플 감정 내용 공유(동의·철회 UX 없이) / **CTS 개발(명시적 재개 시에만)** ← **이 워커 포함**
>
> ⚠️ 두 번째 항목의 전제인 **🔁 루프 탭·루프 계측은 마음풀 전용**(§2.4 미포팅)이므로, CTS 게임에서는 그 판단 근거 데이터를 만들 수단 자체가 없다.
>
> ⚠️ 위 4줄 외의 **서비스별 상세 항목·우선순위·일정**은 외부 메모리 `project_maum_backlog` 에만 존재 — 코드로 복원 불가.
>
> ⚠️ 유지보수 모드 결정의 **경위**(2026-07-18 에 무엇을 보고 그렇게 정했는지)는 외부 메모리 `project_cts_payment_nonprofit` 에만 존재 — 코드로 복원 불가. 이 워커에는 결제 코드가 없으므로(§10), 그 메모리에서 CTS 게임이 상속하는 것은 **"신규 기능 금지"라는 결론 한 줄**뿐이다.
...
- **금지 사항**
  ...
  - 전 서비스 상세 백로그는 **외부 메모리 `project_maum_backlog` 한 곳**에 모여 있다. 레포에 남은 것은 루트 `CLAUDE.md` L77~L82 의 **4줄뿐**이며, 그 원문은 위 §14 머리말에 옮겨 두었다(코드에서 복원). ⚠️ 그 4줄을 넘는 항목·우선순위·일정은 코드로 복원 불가 — 새 작업 지시를 받으면 **먼저 그 메모리를 확인해야 한다.**

## 15. 알려진 리스크 · 기술부채

> ✅ **[BATCH_05 해소 2026-09-19]** R-08(번아웃 metadata 키 — 서버 관대읽기+프론트 energy 추가, 마음게임과 동일) · R-18(`qt` 딥링크 화이트리스트) · R-19(`PHYWEB_URL`→`jesusmaum.com`) 반영·배포(eca55846·75f9488). cron 주석은 미해제(범위 밖).
...
17. **운영 지식의 일부는 여전히 외부 메모리에만 있다.** 2026-09-19 작업으로 **레포에 근거가 남아 있는 것은 전부 본문에 옮겼다**(§14). 남은 공백은 **"왜 그렇게 정했는가" = 의사결정 맥락**이며, 이는 원리상 코드에 없다.

    | 외부 메모리 | 코드로 복원된 부분 | 복원 불가(의사결정 맥락) |
    |---|---|---|
    | `project_cts_payment_nonprofit` | 유지보수 모드 규칙 원문 2문단(§14 머리말). **이 워커에는 결제 코드가 없어**(§10) 그 외에 상속할 내용이 없다 | 2026-07-18 에 유지보수 모드를 정한 경위, 비영리→영리 전환 판단(본체 쪽 맥락) |
    | `project_maum_backlog` | 루트 `CLAUDE.md` L77~L82 백로그 4줄 원문(§14 머리말) | **전 서비스 상세 백로그 전부**(항목·우선순위·일정) |
    | `feedback_search_whole_codebase` | 재발 방지 규칙 원문(루트 `CLAUDE.md` L68 · `../cts-maum-main/CLAUDE.md` §6-2): *"`ls`로 파일 목록을 봐놓고도 한 파일만 뒤진 게 원인"* → 이 워커에서는 `game_engine.jsx`·`game_registry.jsx`·`game_hub.jsx` + `games/` 9종을 **전부** 훑어야 한다. §15-5(딥링크 화이트리스트)·§15-10(배지)·§15-12(게임명)이 모두 **파일 간 불일치**라는 점이 이 규칙의 근거다 | 2026-07-17 사고의 대화 경위·사용자 지적 원문 |
    | `project_cts_naver_login` · `project_maumful_email_verify` 등 인증 관련 | — | **이 워커에는 자체 인증이 없다**(본체 `?t=` SSO 전용, §8) → 해당 없음. 본체 쪽은 `../cts-maum-main/docs/DESIGN.md` §8.1·§8.2 가 코드에서 복원해 두었다 |

    → **잔여 리스크**: `project_maum_backlog` 는 레포 대응물이 4줄뿐이라 사라지면 복구할 수 없다. 나머지 두 메모리는 **결론이 레포에 인용돼 있어 운영은 계속 가능**하고, 잃는 것은 재검토에 필요한 근거뿐이다.
