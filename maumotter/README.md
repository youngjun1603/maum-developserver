...
3. **D1 생성:** `maumotter-db` → `database_id`를 `wrangler.toml`에 기입 → **Console에서 `schema.sql` 실행**.
...
- ⏭ **인증 분리**: MVP는 users를 maumotter-db에 둠 → 마음곁 합류 시 공용 `maum-auth`로 이관(`maum_user_id` 유지).
