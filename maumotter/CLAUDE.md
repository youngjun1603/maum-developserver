...
**또또 음성(TTS, 2026-06 구축)**: `POST /api/tts`(authed) → **OpenAI `gpt-4o-mini-tts` 우선**(`instructions`로 톤 지시), 실패 시 **`tts-1` 폴백**(`speed:1.0`, instructions 미지원). 답변 텍스트만 합성(아이 발화 미전송). 버디별 음성 **또또=`fable`·라라=`nova`**, KV 캐시 키 `v2|voice|text`(`ttscache:` 30일·인사/공통문구 재생성 0) — **음색·모델 바꾸면 접두어 `v3`로 올려 옛 캐시 무효화**. (R-29 정정: 이전 문서의 "tts-1"·"또또=shimmer"는 코드와 불일치였음) ⚠️ **반드시 AI Gateway 경유**(`OPENAI_GATEWAY` const) — 직접 api.openai.com은 Workers egress에서 **403 `unsupported_country_region_territory`** 차단(Anthropic과 동일). `OPENAI_API_KEY` **설정됨**(sk-ant- 클로드키 아님, OpenAI 별도계정 sk-proj-). 미설정/실패 시 프론트 `speak()`가 **기기 speechSynthesis 폴백**(무영향). 모바일 자동재생: `startConversation`에서 Audio+speechSynthesis **둘 다 잠금해제**(silent wav·빈 utterance). 개인정보 처리위탁에 OpenAI 명시.
...
- **수달 추가 구현(착수 지시 시)**: `POST /api/grant`(HMAC=`MAUM_SSO_SECRET` 검증 → `{email, service, grantType, orderId, amount, exp}`(R-44: 6필드·서명 대상) → maum-auth email→uid[없으면 생성] → 기존 `applyGrant(uid, sub_light|sub_pro|pack10)`) + `external_orders` 멱등테이블 + `/api/grant/revoke`(환불). 기존 쿠폰·SSO·entitlement 재사용.
