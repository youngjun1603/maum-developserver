...
// ── 마음풀 통합결제: 서명 grant 수신 (마음풀 결제성공 → 여기로 지급) ──
//   서명 = 마음풀 signSso(MAUM_SSO_SECRET, {email,service,grantType,orderId,amount,exp}) — verifySso와 동일 스킴.
//   재생 방지 = exp + orderId 멱등(external_orders PK). 선구매 후가입 = email 없으면 계정 생성(SSO와 동일).
async function verifyGrantToken(secret: string, token: string): Promise<any | null> {
  const p = await verifySso(secret, token);
  if (!p) return null;
  if (!p.exp || Number(p.exp) < Math.floor(Date.now() / 1000)) return null; // 만료(재생 방지)
  return p;
}
...
app.post('/api/grant/revoke', async (c) => {
  ...
  if (p.service && p.service !== 'otter') return c.json({ error: 'service mismatch' }, 400);   // R-37: 대상 서비스 검사(/api/grant와 동일)
  const orderId = String(p.orderId || '');
