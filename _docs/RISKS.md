...
| 2026-09-19 | **BATCH_01(보안긴급) 해소**: R-01(리더보드 이메일→user_id)·R-02(weekly IDOR 403)·R-05(share/respond 소유권)·R-23(부부 토큰타입 bubu 한정)·R-42(네이버 state CSRF)·R-43(소셜 verified=1, A+B·실측 1건 정정). 마음풀 본체 동일결함(R-42/43)은 사용자 승인받아 후속 반영. R-43-C(이메일 UI)는 보류. 결과 BATCH_01_결과.md. |
| 2026-09-19 | **BATCH_02(결제정산) 해소**: R-37(관리자환불 이용권 회수 확대·실누수 1곳·S1)·R-38/R-39(CTS 웹훅 토스 재조회+금액검증·S1)·R-40(구독 INSERT 로그 표면화·S1)·R-44(grant 6필드 문서)·R-45(grant amount·service 검증)·R-47(정산 원장 기준 통일)·R-11(쿼터 실패 환불)·R-21(크레딧 반환값 검사). **실측: external_grants/orders/CTS charge 전부 0행=활성손실 0, 전부 잠재·예방수정.** 보류: R-41(라이브전환)·R-13(수신거부)=현행유지. 결과 BATCH_02_결과.md. |
| 2026-09-19 | **BATCH_04(공유규약) 해소**: R-04(_shared/auth.ts 캐논 5함수 복원)·R-09(곁 CORS)·R-24(부부 공유철회+inbox 필터)·R-48(customerKey cts_user)·R-50(주석 IP당). 보류: R-10(곁 SSO 링크 부재)·R-46(grant 키 분리)=설계판단 / R-25(relation_safety, 가정폭력 안전·user_id 컬럼 부재)=현행유지. 결과 BATCH_04_결과.md. |
...
| 2026-09-20 | **BATCH_03 문서 3건 해소**: R-20(`D1_SQL_실행순서.sql`에 couple_sessions CHECK 2개·인덱스명 정본 일치+재실행금지 헤더, 사본 4개로 정정, 운영DB는 이미 CHECK 실측→무변경) · R-34(CTS 마이그레이션 번호 규칙 CTS·마음풀 CLAUDE.md 명문화, rename 0건) · R-51(payments/subscriptions/usage_history 미존재=0004 DROP 원격 실측 확인, DESIGN.md·SETUP.md 정정). 코드·스키마 무변경. |
