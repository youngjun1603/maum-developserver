...
- ⚠️ **R-34 CTS로 마이그레이션 옮길 때 번호 따라가지 마라** — CTS `migrations/`는 **0015부터 번호가 갈라져** 같은 번호가 다른 스키마를 뜻한다(CTS 설계서 §6.4 매핑표). 마음풀 마이그레이션을 **번호로** CTS에 복사 금지. 내용을 옮길 땐 CTS **다음 연번**으로 새로 만들고 첫 줄 주석에 대응 마음풀 번호를 적는다.
