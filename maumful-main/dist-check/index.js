...
app.post("/api/auth/register", async (c) => {
  ...
  const validPartner = partnerCode ? await DB.prepare("SELECT code FROM partners WHERE code=? AND is_active=1").bind(String(partnerCode).toUpperCase()).first() : null;
...
app.post("/api/auth/partner-sso", async (c) => {
  ...
  const { partnerCode, ssoToken } = await c.req.json();
  ...
  const key = await crypto.subtle.importKey(
    "raw",
    enc.encode(partner.sso_secret),
    { name: "HMAC", hash: "SHA-256" },
    false,
    ["sign"]
  );
  const sigBuf = await crypto.subtle.sign("HMAC", key, enc.encode(payloadB64));
  const expectedSig = btoa(String.fromCharCode(...new Uint8Array(sigBuf))).replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
  ...
  ).bind(socialProvider, socialId).first();
  let isNewUser = false;
  if (!user) {
    isNewUser = true;
    const country = (c.req.header("cf-ipcountry") ?? "KR").toUpperCase();
    ...
    await DB.prepare("INSERT INTO credit_transactions (user_id,type,amount,reason,balance_after) VALUES (?,?,?,?,?)").bind(newId, "gain", 20, "signup_bonus", 20).run();
...
app.post("/api/auth/kakao", async (c) => {
  ...
  const info3 = await userRes.json();
  const kakaoId = String(info3.id);
  const email = info3.kakao_account?.email;
  const nickname = info3.kakao_account?.profile?.nickname;
  let user = await DB.prepare("SELECT * FROM users WHERE social_provider = ? AND social_id = ?").bind("kakao", kakaoId).first();
  if (!user) {
    if (email) {
      const existing = await DB.prepare("SELECT * FROM users WHERE email = ?").bind(email.toLowerCase()).first();
      if (existing) {
        await DB.prepare("UPDATE users SET social_provider=?,social_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind("kakao", kakaoId, existing.id).run();
...
app.get("/api/auth/naver/url", (c) => {
  ...
  const serviceUrl = c.env.SERVICE_URL || "";
...
app.get("/api/auth/naver/callback", async (c) => {
  ...
  const errPage = /* @__PURE__ */ __name((msg) => new Response(
  ...
  let user = await DB.prepare("SELECT * FROM users WHERE social_provider=? AND social_id=?").bind("naver", naverId).first();
  if (!user) {
    if (email) {
      const existing = await DB.prepare("SELECT * FROM users WHERE email=?").bind(email.toLowerCase()).first();
      if (existing) {
        await DB.prepare("UPDATE users SET social_provider=?,social_id=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind("naver", naverId, existing.id).run();
...
app.post("/api/auth/forgot-password", async (c) => {
  ...
  await sendPasswordResetEmail(c.env, email.toLowerCase(), userForEmail?.nickname || "", resetToken).catch((e) => console.error("[ForgotPw] \uBA54\uC77C \uBC1C\uC1A1 \uC2E4\uD328:", e));
  return c.json({
    success: true,
...
1\u20132 small, manageable practices for everyday life. No mention of treatment or medication.`;
  ...
  if (req.testType === "PHQ9") {
    ...
    const items = r.items ?? [];
    if (lang === "ko") {
      const formatted2 = items.map((item, idx) => idx + 1 + ". " + item.question + ": " + item.score + "\uC810").join(NL);
      return ctx + NL + NL + "PHQ-9 \uC6B0\uC6B8 \uC790\uAC00\uC810\uAC80 \uACB0\uACFC" + NL + "\uCD1D\uC810: " + total + "/27 (" + level + ")" + NL + NL + "\uBB38\uD56D\uBCC4 \uC751\uB2F5:" + NL + formatted2 + NL + fmt;
    }
  ...
  if (req.testType === "GAD7") {
    ...
    if (lang === "ko") {
      const formatted2 = items.map((item, idx) => idx + 1 + ". " + item.question + ": " + item.score + "\uC810").join(NL);
      return ctx + NL + NL + "GAD-7 \uBD88\uC548 \uC790\uAC00\uC810\uAC80 \uACB0\uACFC" + NL + "\uCD1D\uC810: " + total + "/21 (" + level + ")" + NL + NL + "\uBB38\uD56D\uBCC4 \uC751\uB2F5:" + NL + formatted2 + NL + fmt;
    }
    const formatted = items.map((item, idx) => idx + 1 + ". " + item.question + ": " + item.score).join(NL);
  ...
  if (req.testType === "DASS21") {
    ...
    return ctx + NL + NL + "DASS-21 Results" + NL + "Depression: " + dep?.score + " (" + dep?.level + ") \xB7 Anxiety: " + anx?.score + " (" + anx?.level + ") \xB7 Stress: " + str?.score + " (" + str?.level + ")" + NL + fmt;
  }
  ...
  if (req.testType === "DSI") {
    ...
    if (lang === "ko") {
      const scaleText2 = Object.entries(scales).map(([k, v]) => k + ": " + v + "\uC810").join(NL);
      return ctx + NL + NL + "SDRI \uC790\uAE30\uBD84\uD654 \uBC18\uC751\uC131 \uAC80\uC0AC \uACB0\uACFC" + NL + "\uCD1D\uC810: " + total + "\uC810" + NL + NL + "\uC18C\uCC99\uB3C4\uBCC4 \uC810\uC218:" + NL + scaleText2 + NL + fmt;
    }
  ...
  if (req.testType === "RIASEC") {
    const dominantType = r.dominant_type;
    const scores = r.scores;
    if (lang === "ko") {
      const typeNamesKo = { R: "\uC2E4\uC7AC\uD615", I: "\uD0D0\uAD6C\uD615", A: "\uC608\uC220\uD615", S: "\uC0AC\uD68C\uD615", E: "\uC9C4\uCDE8\uD615", C: "\uAD00\uC2B5\uD615" };
      const scoresText2 = Object.entries(scores).sort(([, a], [, b]) => b - a).map(([t, s]) => typeNamesKo[t] + "(" + t + "): " + s + "/25").join(", ");
      return ctx + NL + NL + "Holland RIASEC \uC9C1\uC5C5 \uD765\uBBF8 \uAC80\uC0AC \uACB0\uACFC" + NL + "\uC6B0\uC138 \uC720\uD615: " + dominantType + "\uD615 (" + dominantType.split("").map((t) => typeNamesKo[t]).join("\xB7") + ")" + NL + "\uC720\uD615\uBCC4 \uC810\uC218: " + scoresText2 + NL + fmt;
    }
    const typeNamesEn = { R: "Realistic", I: "Investigative", A: "Artistic", S: "Social", E: "Enterprising", C: "Conventional" };
    const scoresText = Object.entries(scores).sort(([, a], [, b]) => b - a).map(([t, s]) => typeNamesEn[t] + "(" + t + "): " + s + "/25").join(", ");
    return ctx + NL + NL + "Holland RIASEC Career Interest Assessment" + NL + "Dominant Type: " + dominantType + " (" + dominantType.split("").map((t) => typeNamesEn[t]).join("\xB7") + ")" + NL + "Scores: " + scoresText + NL + fmt;
  }
  if (req.testType === "VALUES") {
    const scores = r.scores;
    const sorted = Object.entries(scores).sort(([, a], [, b]) => b - a);
    if (lang === "ko") {
      const top32 = sorted.slice(0, 3).map(([k, s]) => k + ": " + s + "\uC810").join(", ");
      const allText2 = sorted.map(([k, s]) => k + ": " + s + "\uC810").join(NL);
      return ctx + NL + NL + "\uC9C1\uC5C5\uAC00\uCE58\uAD00 \uAC80\uC0AC \uACB0\uACFC" + NL + "\uC0C1\uC704 3\uAC1C \uAC00\uCE58: " + top32 + NL + NL + "\uC804\uCCB4 \uAC00\uCE58\uC694\uC778:" + NL + allText2 + NL + fmt;
    }
    const valuesNamesEn = {
...
app.get("/api/user/daily-context", async (c) => {
  ...
  const user = await DB.prepare("SELECT nickname FROM users WHERE id=?").bind(userId).first();
  ...
  if (tests.length > 0) {
    const testSummary = tests.map((t) => {
      const days = Math.floor((Date.now() - new Date(t.performed_at).getTime()) / 864e5);
      return `${t.test_type}(${days}\uC77C \uC804, ${t.score}\uC810${t.level ? "/" + t.level : ""})`;
    }).join(", ");
    parts.push(`\uCD5C\uADFC \uAC80\uC0AC: ${testSummary}`);
  }
  if (moods.length > 0) {
    const MOOD_MAP = { 1: "\uAE30\uC068", 2: "\uD3C9\uC628", 3: "\uBB34\uAC10\uAC01", 4: "\uC2AC\uD514", 5: "\uBD88\uC548", 6: "\uD654\uB0A8", 7: "\uC9C0\uCE68" };
    const moodList = moods.map((m) => {
      try {
        const meta = JSON.parse(m.metadata || "{}");
        return MOOD_MAP[meta.mood] || "\uAE30\uB85D\uB428";
      } catch {
        return "\uAE30\uB85D\uB428";
      }
    }).join(", ");
  ...
  if (checkin) {
    const days = Math.floor((Date.now() - new Date(checkin.created_at).getTime()) / 864e5);
...
app.get("/api/test/recent-summary", async (c) => {
  ...
  const summary = tests.map(
    (r) => `${r.test_type} (${r.performed_at.slice(0, 10)}): ${r.score}\uC810${r.ai_analysis ? " \u2014 " + r.ai_analysis.slice(0, 80) : ""}`
  ).join("\n");
...
app.post("/api/test/external-result", async (c) => {
  ...
  if (score === void 0 || score === null || typeof score !== "number") {
    return c.json({ error: "\uC810\uC218\uB97C \uC785\uB825\uD574 \uC8FC\uC138\uC694" }, 400);
  }
  const performedAt = conductedAt || (/* @__PURE__ */ new Date()).toISOString();
...
**\uC77C\uC0C1 \uC18D \uD328\uD134** \u2014 \uB300\uC778\uAD00\uACC4\xB7\uC5C5\uBB34\xB7\uAC10\uC815 \uC870\uC808\uC5D0\uC11C \uC5B4\uB5BB\uAC8C \uB098\uD0C0\uB0A0 \uC218 \uC788\uB294\uC9C0
...
(FOLLOWUP\uC740 \uB9C8\uC74C\uD480 \uB0B4 \uAC80\uC0AC \uC911 \uC774 \uACB0\uACFC\uC640 \uAD00\uB828 \uAE4A\uC740 \uAC83: PHQ9/GAD7/BURNOUT/BIG5/LOST/DSI)`;
  const truncated = pdfText.slice(0, 15e3);
...
${truncated}`;
  ...
  const suggestedGames = gamesMatch ? gamesMatch[1].replace(/"/g, "").split(",").map((s) => s.trim()).filter(Boolean) : ["mood", "garden"];
  const followUpTests = followupMatch ? followupMatch[1].replace(/"/g, "").split(",").map((s) => s.trim()).filter(Boolean) : [];
  ...
  try {
    ...
    ).bind(userId, testType.toUpperCase().slice(0, 20), analysis.slice(0, 1e3)).run();
  } catch {
    await DB.prepare(
...
app.post("/api/chat/mood-log", async (c) => {
  ...
  await DB.prepare("INSERT INTO mood_logs (user_id, mood_score, test_type) VALUES (?, ?, ?)").bind(userId, score, testType ?? null).run();
  return c.json({ success: true });
});
...
app.get("/api/test/cbt-plan", async (c) => {
  ...
  if (cached && cached.generatedAt >= new Date(Date.now() - 30 * 86400 * 1e3).toISOString().slice(0, 10)) {
...
{"summary":"...","plan":[{"week":1,"title":"...","theme":"...","practice":"...","game":"...","tip":"..."},...]}`;
  try {
    ...
    const aiData = await aiRes.json();
    const raw2 = (aiData?.content?.[0]?.text || "").trim();
    let jsonStr;
    if (raw2.startsWith("{")) {
      jsonStr = raw2;
    } else {
      const si = raw2.indexOf("{");
      const ei = raw2.lastIndexOf("}");
      if (si === -1 || ei === -1) return c.json({ success: false, error: "AI \uC751\uB2F5\uC5D0\uC11C JSON\uC744 \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4. \uC7A0\uC2DC \uD6C4 \uB2E4\uC2DC \uC2DC\uB3C4\uD574 \uC8FC\uC138\uC694." }, 502);
      jsonStr = raw2.slice(si, ei + 1);
    }
    ...
    return c.json({ success: false, error: e?.message || "\uD50C\uB79C \uC0DD\uC131 \uC2E4\uD328" }, 500);
  }
});
app.get("/api/ai-chat/memory", async (c) => {
  ...
  const userId = await getAuthUserId(c.req.raw, KV);
  if (!userId) return c.json({ success: false, error: "\uB85C\uADF8\uC778 \uD544\uC694" }, 401);
  const memStore = await KV.get(`chat_mem:${userId}`, "json");
  ...
  if (isGuest) {
    ...
    if (guestUsed >= GUEST_TOTAL_LIMIT) {
      return c.json({
        ...
        totalUsed: guestUsed,
        totalLimit: GUEST_TOTAL_LIMIT,
        needsSignup: true,
        errorCode: "guest_limit_exceeded"
      }, 429);
    }
    await KV.put(guestKey, String(guestUsed + 1));
  } else {
    const userRow = await DB.prepare("SELECT email, credits FROM users WHERE id=?").bind(userId).first();
...
${dailyContext}` : "";
  ...
  let hasMemory = false;
  if (memKey) {
    const memStore = await KV.get(memKey, "json");
...
[\uC774\uC804 \uC0C1\uB2F4 \uAE30\uC5B5 (${mem.date})]
${mem.points.map((p) => `\u30FB${p}`).join("\n")}`;
    }
    if (messages.length >= 4) {
      const msgArr = messages;
      const userPts = msgArr.filter((m) => m.role === "user").slice(-5).map((m) => String(m.content).slice(0, 120));
      ...
      try {
        c.executionCtx.waitUntil(saveP);
      } catch {
        saveP.catch(() => {
        });
      }
...
async function sendEmail(env2, to, subject, html) {
  ...
  try {
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: env2.RESEND_FROM_EMAIL || "\uB9C8\uC74C\uD480 <noreply@maumful.com>",
        to: [to],
        subject,
        html
      })
...
app.post("/api/credits/prepare-charge", async (c) => {
  ...
  const pkg = PACKAGES[packageKey];
  if (!pkg) return c.json({ success: false, error: "\uC798\uBABB\uB41C \uD328\uD0A4\uC9C0" }, 400);
  const userRow = await DB.prepare("SELECT partner_code FROM users WHERE id=?").bind(userId).first();
  const partnerCode = userRow?.partner_code ?? null;
  const r = await DB.prepare("INSERT INTO credit_charges (user_id,package_key,credits,amount,currency,pg,partner_code) VALUES (?,?,?,?,?,?,?)").bind(userId, packageKey, pkg.credits, pkg.amount, pg === "stripe" ? "USD" : "KRW", pg, partnerCode).run();
...
<head>
  ...
  <meta name="description" content="PHQ-9\xB7GAD-7\xB7Big5 \uB4F1 8\uC885 \uC2EC\uB9AC\uAC80\uC0AC\uB97C \uBB34\uB8CC\uB85C \uC2DC\uC791\uD558\uC138\uC694. AI \uC0C1\uB2F4\uC73C\uB85C \uB0B4 \uB9C8\uC74C\uC744 \uB354 \uAE4A\uC774 \uC774\uD574\uD558\uACE0, \uC81C\uD734 \uC0C1\uB2F4\uC0AC\uC640 \uC9C1\uC811 \uC5F0\uACB0\uB429\uB2C8\uB2E4.">
  ...
  <meta property="og:description" content="PHQ-9\xB7Big5 \uB4F1 8\uC885 \uC2EC\uB9AC\uAC80\uC0AC \uBB34\uB8CC \uC2DC\uC791. \uAC00\uC785 \uC989\uC2DC 20 \uD06C\uB808\uB527 \uC9C0\uAE09 \xB7 AI \uC0C1\uB2F4 \xB7 \uC81C\uD734 \uC0C1\uB2F4\uC0AC \uC5F0\uACB0.">
  ...
  <meta name="twitter:description" content="PHQ-9\xB7Big5 \uB4F1 8\uC885 \uC2EC\uB9AC\uAC80\uC0AC. \uAC00\uC785 \uC989\uC2DC 20 \uD06C\uB808\uB527 \uBB34\uB8CC.">
  ...
  <script type="application/ld+json">{
    ...
    "@graph": [
      ...
      },
      {
        ...
        "operatingSystem": "Web",
        "description": "PHQ-9\xB7GAD-7\xB7Big5 \uB4F1 8\uC885 \uC2EC\uB9AC\uAC80\uC0AC\uC640 AI \uC0C1\uB2F4 \uC11C\uBE44\uC2A4",
      ...
      },
      {
        "@type": "FAQPage",
        "mainEntity": [
          { "@type": "Question", "name": "\uC2EC\uB9AC\uAC80\uC0AC \uBE44\uC6A9\uC740 \uC5BC\uB9C8\uC778\uAC00\uC694?", "acceptedAnswer": { "@type": "Answer", "text": "\uAC00\uC785 \uC989\uC2DC 20 \uD06C\uB808\uB527\uC774 \uBB34\uB8CC \uC9C0\uAE09\uB418\uBA70, PHQ-9\xB7GAD-7\xB7DASS-21\uC740 \uBB34\uB8CC\uB85C \uC774\uC6A9\uD560 \uC218 \uC788\uC2B5\uB2C8\uB2E4." } },
...
app.get("/api/nearby-counseling", async (c) => {
  ...
  const radius = Number(c.req.query("radius") || 3e3);
  ...
  if (shareTestResult) {
    if (testSummary) {
      ...
      if (recentTests.results && recentTests.results.length > 0) {
        finalTestSummary = recentTests.results.map(
          (r2) => `${r2.test_type} (${r2.performed_at.slice(0, 10)}): ${r2.score}\uC810${r2.ai_analysis ? " \u2014 " + r2.ai_analysis.slice(0, 80) : ""}`
        ).join("\n");
      }
    }
  }
...
async function hkdfDerive(ikm, salt, info3, length) {
  const key = await crypto.subtle.importKey("raw", ikm, { name: "HKDF" }, false, ["deriveBits"]);
  const bits = await crypto.subtle.deriveBits({ name: "HKDF", hash: "SHA-256", salt, info: info3 }, key, length * 8);
  return new Uint8Array(bits);
}
__name(hkdfDerive, "hkdfDerive");
async function encryptWebPush(plaintext, p256dhB64, authB64) {
  const enc = new TextEncoder();
  const receiverPubRaw = b64urlDecode(p256dhB64);
  ...
  const keyInfo = concatBytes(enc.encode("WebPush: info\0"), receiverPubRaw, senderPubRaw);
  const prk = await hkdfDerive(dh, authSecret, keyInfo, 32);
  const cek = await hkdfDerive(prk, salt, concatBytes(enc.encode("Content-Encoding: aes128gcm\0"), new Uint8Array([0, 1])), 16);
  const nonce = await hkdfDerive(prk, salt, concatBytes(enc.encode("Content-Encoding: nonce\0"), new Uint8Array([0, 1])), 12);
...
async function signVapidJwt(endpoint, vapidPubB64, vapidPrivB64) {
  const enc = new TextEncoder();
  ...
  const privBytes = b64urlDecode(vapidPrivB64);
  const jwk = {
    kty: "EC",
    crv: "P-256",
...
async function sendWebPushMessage(sub, notification, vapidPubB64, vapidPrivB64) {
  try {
...
__name(sendWebPushMessage, "sendWebPushMessage");
async function sendPushToUser(db, env2, userId, notification) {
  const vapidPub = env2.VAPID_PUBLIC_KEY;
  const vapidPriv = env2.VAPID_PRIVATE_KEY;
...
app.post("/api/counseling/onboarding", async (c) => {
  ...
  const notifyEmail = c.env.COUNSELING_NOTIFY_EMAIL;
  if (notifyEmail) {
    sendEmail(
      c.env,
      notifyEmail,
      `[\uB9C8\uC74C\uD480] \uC0C1\uB2F4\uC13C\uD130 \uC785\uC810 \uC2E0\uCCAD \u2014 ${center_name}`,
      `<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:24px">
        <h2 style="color:#2D6A4F">\u{1F33F} \uC0C1\uB2F4\uC13C\uD130 \uC785\uC810 \uC2E0\uCCAD</h2>
  ...
  for (const sub of subs.results) {
    ...
    try {
      ...
      if (res.ok && result.paymentKey) {
        ...
        sendPushToUser(DB, env2, sub.user_id, {
          title: "\uB9C8\uC74C\uD480 \uAD6C\uB3C5 \uAC31\uC2E0 \uC644\uB8CC",
...
async function handleDailyReminder(env2) {
  const DB = env2.DB;
  const vapidPub = env2.VAPID_PUBLIC_KEY;
  if (!vapidPub) return;
  const rows = await DB.prepare(`
  ...
  for (const row of rows.results || []) {
    await sendPushToUser(DB, env2, row.user_id, {
...
__name(handleDailyReminder, "handleDailyReminder");
...
app.get("/api/admin/partner-stats", async (c) => {
  ...
  const toDate = to ?? (/* @__PURE__ */ new Date()).toISOString().slice(0, 10);
  const [userStats, chargeStats, dailySignups, dailyRevenue] = await Promise.all([
    ...
    `).bind(fromDate, toDate, code).first(),
    DB.prepare(`
  ...
  const partner = await DB.prepare("SELECT name, revenue_share_rate FROM partners WHERE code=?").bind(code).first();
...
app.get("/api/admin/partner-settlement", async (c) => {
  ...
  const denied = adminGuard(c);
  if (denied) return c.json({ success: false, error: denied }, denied === "Forbidden" ? 403 : 401);
  const { code, month } = c.req.query();
  if (!code || !month) return c.json({ success: false, error: "code, month \uD30C\uB77C\uBBF8\uD130 \uD544\uC218 (month: YYYY-MM)" }, 400);
  const partner = await DB.prepare("SELECT code, name, revenue_share_rate, contact_email FROM partners WHERE code=?").bind(code.toUpperCase()).first();
  if (!partner) return c.json({ success: false, error: "\uD30C\uD2B8\uB108\uB97C \uCC3E\uC744 \uC218 \uC5C6\uC2B5\uB2C8\uB2E4." }, 404);
  ...
  const [newUsers, charges] = await Promise.all([
    ...
    `).bind(code.toUpperCase(), fromDate, toDate).first(),
    DB.prepare(`
