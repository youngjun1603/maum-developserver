...
4. **영상 미저장**(✅ 구현): 보호자가 ~7초 촬영 → 기기에서 프레임 6장 추출 → **프레임만 분석에 일시 전송(Claude 비전)** → **결과 리포트만 저장, 영상 원본·프레임은 어디에도 저장하지 않음**. 반려동물 정보라 아동(마음수달)보다 민감도 낮으나, 이슈 없는 한 미저장 원칙 유지(사용자 결정 2026-06).
...
- **2차:** ✅ 짧은 영상 통역(프레임 추출→비전 분석·미저장) 구현 / ⬜ 행동 도감 화면·개 사전 확장·관찰 누적 추이.
