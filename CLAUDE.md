...
- **선행조건 대기**: 앱화·통합해석 상품화 → **토스 실결제 반영 후**. (연동형 통합결제는 **발신·수신 배포 완료** — 아래 절 참조, 남은 건 라이브 결제키 전환)
- **금지**: 커플 감정 내용 공유(동의·철회 UX 없이) / CTS 개발(명시적 재개 시에만)

---

## 연동형 유료결제 (통합결제) — 발신·수신 배포 완료 (R-31·R-55 정정 2026-09-20)
수달·곁·부부·**세대**·**phyweb** 유료결제를 **마음풀에서 상품으로 판매 → 결제내역을 각 서비스로 자동 전달(grant)** 하는 방식. **발신(마음풀)·수신(수달·곁·phyweb) 코드 모두 배포 상태**이며 E2E 검증 완료(지급·멱등·위조401·환불revoke — `maumful-main/CLAUDE.md` 상품/결제 절). ~~토스 완전 반영 전까지 커밋·푸시·배포 금지~~ → **이 금지는 해제됨.** 남은 제약은 **토스 라이브 결제키 전환 시점**뿐이다(현재 테스트키 — 카탈로그·전환로직은 다 살아 있고 실결제만 대기).
- ⚠️ 세대 통역팩(`sedae_pack10/20/40`)은 `service` 필드가 없는 **공용 크레딧 지급 상품**이라 grant 경로가 아니라 maumful `users.credits` 직접 지급이다(부부·세대 3단계, 2026-08-08 확장 — `maumful-main/CLAUDE.md` 참조). grant(A안)를 쓰는 건 별도 생태계인 **수달·곁·phyweb**뿐.
- **결제 표기 = 하이브리드**(내부 크레딧, 겉은 명명 상품 — 선불충전금/PG 기피 회피). 마음풀·부부는 이미 이 구조. 수달·곁은 별도 생태계라 `applyGrant`(sub/pack)로 지급.
- **전달 = A안(서명 grant API)**: 마음풀 결제성공 → 대상 서비스 `POST /api/grant`(HMAC=MAUM_SSO_SECRET, `{email, service, grantType, orderId, amount, exp}`) → email로 maum-auth 계정 조회/생성 → `applyGrant`. 멱등·환불 revoke·선지급 재시도 포함. ⚠️ **R-44: 페이로드는 6필드**(서명 대상이 payload JSON이라 필드·순서가 하나라도 다르면 전건 401). **grant·revoke 동일 6필드.** 상세 계약은 각 서비스 `docs/DESIGN.md` §10.1 표가 원본.
