...
- **연동형 유료결제**: 부부 유료는 마음풀 상점에서 **부부 상품 카드 → 내부 maumful 크레딧 지급**(하이브리드, 별도 테이블 없음). 설계 완료·**토스 반영 후 착수**. 상세 = 메모리 `project_maum_unified_payment` / 루트 CLAUDE.md 「연동형 유료결제」. **착수 지시 전까지 구현·커밋 금지.**
