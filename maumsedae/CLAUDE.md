...
## 감수 대기 (기획자 — 개발과 병행)
1. **공경과 분리 렌즈 최종 문구** — 기독교 상담 전문가 감수(현재 DEV_01 초안 사용, 코드에 명시). SPEC 7장이 *"높음(윤리 전제)"* 로 분류.
2. **청소년 케이스뱅크** — ✅ **초안 완료**: `casebank/BATCH_01_TEEN.md`(6케이스, 실엔진 출력). **감수 대기**.
   - ★ 미해결 쟁점(실측): **T02(모호한 반복)·T03(소지품 수색·또래 차단)이 T2로 발동하지 않는다.** 청소년 모듈의 "민감도 상향"과 마음부부 감수의 "T2 과발동 방지"가 **정면 충돌**. 아이한테는 어느 쪽이 맞는지 감수가 결정.
   - ★ teen 응답 **말투가 존댓말/반말 사이에서 흔들림**(T1·T2는 반말, T3는 존댓말 경향) — 통일 방향 판단 필요.
3. ~~거점 마케팅 타겟~~ → **1차 폭풍으로 확정**(위 참조).
