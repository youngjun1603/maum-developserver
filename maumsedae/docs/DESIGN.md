...
>
> 상세 이력·기획 진본 전반은 외부 메모리 `project_maumbubu`(마음세대 항목) 참조 — **문서화 필요**.
> 전 서비스 백로그는 외부 메모리 `project_maum_backlog` 참조 — **문서화 필요**.
> 연동형 유료결제 실행 스펙은 외부 메모리 `project_maum_unified_payment` 참조 — **문서화 필요**.
...
- **연동형 유료결제(통합결제)**: 설계 완료·착수 대기. **토스 실결제 완전 반영 전까지 관련 코드 커밋·푸시·배포 금지**(루트 `CLAUDE.md`). 상세는 외부 메모리 `project_maum_unified_payment` 참조 — **문서화 필요**.
...
- **미착수 / 대기**
  ...
  | 연동형 유료결제(통합결제) | 대기 | **토스 실결제 반영 후**. 메모리 `project_maum_unified_payment` |
