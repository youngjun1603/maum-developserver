...
- **검사 종류의 진본은 마음풀이다** — 문항 수·척도 구성·카드 순서·채점 구간표는 **→ 마음풀 설계서 §2.1 「검사 = 10종 (12종 아님)」**에 복원돼 있다. 마음커플이 실제로 쓰는 것은 그중 **BIG5 · LOST · DSI 3종의 `result_json` 뿐**이며(`SELECT … WHERE test_type IN ('BIG5','LOST','DSI') AND result_json IS NOT NULL`, `src/index.tsx` L339~L349), 조합 7종은 이 3종의 부분집합이다. 마음커플은 검사를 시행하지도, 문항을 갖지도 않는다. ⚠️ 검사 선정 이유·문항 출처는 외부 메모리 `project_maumful_tests` 에만 존재 — 코드로 복원 불가(마음풀 설계서 §15 대조표와 동일).
...
- **크레딧·구독 처리**:
  ...
  - **연동형 유료결제와 마음커플의 실제 관계**(코드에서 복원 · 근거: `maumful-main/src/index.tsx` `PACKAGES` L3118~L3157 전량 확인 · `public/static/app.jsx` `PACKAGES_KR` L6020~L6039 · `SERVICE_GROUPS` L6057~L6063):
    - **마음풀 `PACKAGES` 에 마음커플 상품 키가 하나도 없다.** 부부·세대는 `bubu_pack10/20/40`·`sedae_pack10/20/40`(각 25·50·100cr / 3,300·4,900·8,900원)을 갖고, 수달·곁·phyweb 은 `credits:0` + `service`/`grantType` 로 외부 grant를 받는다. **마음커플만 전용 상품이 없다** — 상점 `SERVICE_GROUPS` 도 `maumful`·`bubu`·`sedae`·`otter`·`gyeot` **5개 그룹뿐이고 커플 그룹이 없다**. 마음커플 사용자는 범용 크레딧(`starter_kr` 50cr/4,900원 · `standard_kr` 120cr/9,900원 · `premium_kr` 300cr/15,000원 · `pro_kr` 700cr/25,000원)으로만 충전한다.
    - 루트 `CLAUDE.md` 「연동형 유료결제」 원문도 대상 서비스를 **"수달·곁·부부"** 로만 열거하고 마음커플을 넣지 않았다:
    ...
    - **결론(코드 기준)**: 마음커플은 마음풀 D1의 `users.credits` 를 직접 차감하므로 grant 경로가 애초에 필요 없다. 연동형 유료결제가 착수돼도 **마음커플 워커에 반영할 코드는 없다**. 위 §10 "마음커플 Plus ₩9,900" 은 결제 대상이 아니라 **대기자 등록 카드**다 — 버튼이 `POST /api/credits/notify-plan` 을 호출하고 "오픈 알림을 신청했습니다"만 띄운다(`app.jsx` L6299~L6302·L6335~L6341). 같은 화면의 주석도 "구독 플랜은 토스페이먼츠 심사 완료 후 정식 출시"라고 적고 있다.
    - ⚠️ **마음커플 전용 상품을 만들지 않기로 한 판단인지, 아직 안 만든 것인지**는 코드로 구분되지 않는다 — 외부 메모리 `project_maum_unified_payment` 에만 존재. 확장 범위·착수 조건·DB/API 계약 검증 절차도 같다. **복원 불가.**
...
- **미착수 / 대기**
  ...
  - **전 서비스 백로그** — 루트 `CLAUDE.md` 「남은 작업 (백로그)」 **원문 중 마음커플 해당 항목만**(코드로 복원 불가 → 원문 인용으로 대체):
    > - **선행조건 대기**: 앱화·통합해석 상품화·연동형 통합결제 → 모두 **토스 실결제 반영 후**
    > - **금지**: 커플 감정 내용 공유(동의·철회 UX 없이) / CTS 개발(명시적 재개 시에만)
    - "커플 감정 내용 공유 금지"는 **이 서비스를 직접 지목한 유일한 백로그 항목**이며 §11.1에 원문이 다시 인용돼 있다. "통합해석 상품화"의 실물은 마음풀 상품 `integrated_one`(40cr / 4,500원, "통합 심층 해석 1회")이고 상점 그룹상 **마음풀 소관**(`SERVICE_GROUPS.maumful`)이다 — 마음커플 몫이 아니다.
    - 나머지 항목(상담사 승인 레거시 제거 · 주간 리포트 메일 실수신 검증 · 마음게임 콘텐츠 확장)은 마음커플 소관이 아니다.
...
> 📄 **[R-54 문서화 2026-09-19]** 마음풀 구독 UI 전체가 **표시 전용**이다. 프론트 플랜 카드(마음풀 Plus ₩5,900 / **마음커플 Plus ₩9,900** / 마음가족 ₩14,900, `app.jsx:6293~`)와 백엔드 `SUBSCRIPTION_PLANS`(basic ₩3,900 / standard ₩8,900 / pro ₩19,900, `maumful src:3254~`)는 **키가 하나도 겹치지 않는다**. "마음커플 Plus ₩9,900" 버튼은 결제가 아니라 `POST /api/credits/notify-plan`(KV `plan_notify:{email}` 90일 TTL) 대기자 등록 후 "오픈 알림 신청" 알럿만 띄운다(§10 상세). 마음커플 워커에는 `notify-plan`·구독 코드가 0건 — 손댈 것 없음.
...
2. **스키마 사본 4개** — `couple_sessions` DDL이 `package/maumcouple/migrations/0001_couple_schema.sql` · `maumful-main/migrations/0010_couple_sessions.sql` · `package/D1_SQL_실행순서.sql` · **CTS(4번째 사본)** 에 있다. 한쪽만 고치면 소리 없이 갈라진다. 소유권을 한 곳으로 정해야 한다. ✅ **[R-20 해소 2026-09-20]** `D1_SQL_실행순서.sql`에 빠져 있던 CHECK 2개(test_type·status)·인덱스명(`idx_test_history_type`)을 정본과 일치시키고 "재실행 금지 아카이브" 헤더 추가. **운영 DB는 이미 CHECK 포함(실측 확인)이라 무변경** — SQLite는 CHECK 사후추가 불가라 재생성 안 함.
...
2026-09-19 작업으로 이 문서의 외부 메모리 참조 2곳을 코드에서 복원하고, 검사 종류는 마음풀 설계서로 위임했다. 오른쪽 열은 원리상 코드에 없는 것(= 의사결정 맥락)이다.

| 외부 메모리 | 코드에서 복원된 부분 | 복원 불가(의사결정 맥락) |
|---|---|---|
| `project_maum_unified_payment` | **마음커플 전용 상품 키가 `PACKAGES` 에 0건**(상점 그룹 5개에도 커플 없음) · 루트 `CLAUDE.md` 원문이 "수달·곁·부부"만 열거 · "마음커플 Plus"는 결제가 아닌 **대기자 등록 카드**(`/api/credits/notify-plan`) · 마음커플은 `users.credits` 직접 차감이라 grant 불필요 | 커플 전용 상품을 **안 만들기로 한 것인지 아직 안 만든 것인지** · 확장 범위·착수 조건 · DB/API 계약 검증 절차 |
| `project_maum_backlog` | — (루트 `CLAUDE.md` 백로그 원문 중 커플 해당 2줄만 · "통합해석 상품화" = 마음풀 `integrated_one` 소관임을 코드로 확인) | **전 서비스 통합 백로그 전부** · 서비스 간 우선순위·일정 |
| `project_maumful_tests` | **→ 마음풀 설계서 §2.1 로 위임.** 이 서비스가 실제로 쓰는 것은 `BIG5`·`LOST`·`DSI` 3종의 `result_json` 뿐(§2.2) | 검사 선정 이유·문항 출처(마음풀 설계서 §15 대조표와 동일) |
